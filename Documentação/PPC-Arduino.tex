...
Para utilizar o sensor de infravermelho, é necessário instalar uma biblioteca do Arduino. Para isso selecione a opção \textit{sketch} > \textit{incluir biblioteca} > \textit{gerenciar bibliotecas}. No campo de busca insira o nome \textbf{IRremote} e instale a biblioteca \textbf{IRremote by shirriff}. Para esse experimento foi utilizada a versão 2.2.3 da biblioteca. A Figura \ref{figBiblioteca} mostra a biblioteca instalada.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{Imagens/Experimentos/9-ControleRemoto/biblioteca.png}
	\caption{Instalação da biblioteca IRremote.}
	\label{figBiblioteca}
\end{figure}
